...
//	memset(WriteBuffer, 0, 2);
	WriteBuffer[0] = reg+2; // +2 to set write bit
	//write val via SPI
	WriteBuffer[1] = val;
	//read return code from SPI peripheral (see Xilinx examples) 
	int return_code = XSpi_Transfer(&SpiInstance, WriteBuffer, NULL, 2);
	//if return code != 0 print an error
	if (return_code != 0) {
		xil_printf("Error! MAXreg_wr, XSpi_Transfer\n");
	}
...
//	memset(ReadBuffer, 0, nbytes+1);
//	memset(WriteBuffer, 0, nbytes+1);
	...
	int return_code = XSpi_Transfer(&SpiInstance, WriteBuffer, NULL, nbytes+1);
	//if return code != 0 print an error
	if (return_code != 0) {
		xil_printf("Error! MAXbytes_wr, XSpi_Transfer\n");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 0);
//	xil_printf("leaving MAXbytes_wr\n");
	return (data + nbytes);
}
...
//	memset(WriteBuffer, 0, 2);
	...
	ReadBuffer[0] = reg;
	//read val via SPI
	int return_code = XSpi_Transfer(&SpiInstance, ReadBuffer, ReadBuffer, 2);
	//read return code from SPI peripheral 
	//if return code != 0 print an error
	if (return_code != 0) {
		xil_printf("Error! MAXreg_rd, XSpi_Transfer\n");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	//return val
...
//	xil_printf("inside MAXbytes_rd\n");
	//psuedocode:
	//select MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 1);

	//write reg via SPI
	u8 ReadBuffer[1 + nbytes];
	u8 WriteBuffer[1 + nbytes];
//	memset(ReadBuffer, 0, 1 + nbytes);
//	memset(WriteBuffer, 0, 1 + nbytes);
	...
	//read return code from SPI peripheral
	int return_code = XSpi_Transfer(&SpiInstance, ReadBuffer, ReadBuffer, nbytes+1);
	//if return code != 0 print an error
	if (return_code != 0) {
		xil_printf("Error! MAXbytes_wr, XSpi_Transfer\n");
	} else {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 0);
//		xil_printf("leaving MAXbytes_rd\n");
	return (data + nbytes);
...
void MAX3421E_reset(void) {
	...
	//hardware reset, then software reset            BallY <= Ball_Y_next;  // Update ball position
