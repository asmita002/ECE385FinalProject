...
module LVL_1_BGD_example (
    input logic [9:0] CharX, CharY, Char_size,
	input logic vga_clk,
...
assign char_left = CharX - Char_size;
assign char_top = CharY - Char_size;

int draw_sprite_x;
int draw_sprite_y;

assign draw_sprite_x = DrawX - char_left;
assign draw_sprite_y = DrawY - char_top;

assign bg_rom_address = ((DrawX * 160) / 640) + (((DrawY * 160) / 480) * 160);
//assign char_rom_address = ((DrawX*32) / 640) + (((DrawY * 32) / 480) * 32);
assign char_rom_address = (draw_sprite_x) + ((draw_sprite_y) * 32);

    always_comb
    begin:Char_on_proc
        if ( draw_sprite_x >= 0 && draw_sprite_x < 32 && draw_sprite_y >= 0 && draw_sprite_y < 32 )
            char_on = 1'b1;
        else 
            char_on = 1'b0;
     end
